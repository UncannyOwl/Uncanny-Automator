...
/**
 * Class Aioseo_Set_Seo_Title
 *
 * @package Uncanny_Automator
 * @method \Uncanny_Automator\Integrations\Aioseo\Aioseo_Helpers get_item_helpers()
 */
class Aioseo_Set_Seo_Title extends \Uncanny_Automator\Recipe\Action {
	...
	protected function setup_action() {
		...
		$this->set_action_code( 'AIOSEO_SET_SEO_TITLE' );
		$this->set_action_meta( 'AIOSEO_POST' );
		$this->set_is_pro( false );
		$this->set_requires_user( false );
		$this->set_background_processing( true );
		// translators: %1$s is the post, %2$s is the SEO title value.
		$this->set_sentence( sprintf( esc_html_x( "Set {{a post's:%1\$s}} SEO title to {{a specific value:%2\$s}}", 'All in One SEO', 'uncanny-automator' ), $this->get_action_meta(), 'SEO_TITLE_VALUE:' . $this->get_action_meta() ) );
		$this->set_readable_sentence( esc_html_x( "Set {{a post's}} SEO title to {{a specific value}}", 'All in One SEO', 'uncanny-automator' ) );
		$this->set_action_tokens(
			array(
				...
				'SEO_TITLE'  => array(
					'name' => esc_html_x( 'SEO title', 'All in One SEO', 'uncanny-automator' ),
					'type' => 'text',
				),
			),
	...
	public function options() {
		...
		$options[] = array(
			...
			'label'       => esc_html_x( 'SEO title', 'All in One SEO', 'uncanny-automator' ),
			'input_type'  => 'text',
			'required'    => true,
		);
	...
	protected function process_action( $user_id, $action_data, $recipe_id, $args, $parsed ) {
		...
		\AIOSEO\Plugin\Common\Models\Post::savePost( $post_id, array( 'title' => $seo_title ) );
