...
/**
 * Class Yoast_Delete_Seo_Description
 *
 * @package Uncanny_Automator
 * @method \Uncanny_Automator\Integrations\Wordpress_Seo\Wordpress_Seo_Helpers get_item_helpers()
 */
class Yoast_Delete_Seo_Description extends \Uncanny_Automator\Recipe\Action {
	...
	protected function setup_action() {
		...
		$this->set_action_code( 'YOAST_DELETE_SEO_DESCRIPTION' );
		$this->set_action_meta( 'YOAST_POST' );
		$this->set_is_pro( false );
		$this->set_requires_user( false );
		$this->set_background_processing( true );
		// translators: %1$s is the post.
		$this->set_sentence( sprintf( esc_html_x( "Delete {{a post's:%1\$s}} SEO description", 'Yoast SEO', 'uncanny-automator' ), $this->get_action_meta() ) );
		$this->set_readable_sentence( esc_html_x( "Delete {{a post's}} SEO description", 'Yoast SEO', 'uncanny-automator' ) );
	...
	protected function process_action( $user_id, $action_data, $recipe_id, $args, $parsed ) {
		...
		$this->get_item_helpers()->delete_yoast_meta( $post_id, 'metadesc' );
