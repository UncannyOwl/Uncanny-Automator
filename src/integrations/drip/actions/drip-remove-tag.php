 ...
 */
class DRIP_REMOVE_TAG extends \Uncanny_Automator\Recipe\App_Action {
	...
	public function setup_action() {
		...
		$this->set_action_code( 'REMOVE_TAG' );
		...
		$this->set_readable_sentence( esc_attr_x( 'Remove {{a tag}} from {{a subscriber}}', 'Drip', 'uncanny-automator' ) );
		$this->set_sentence(
			sprintf(
				// translators: %1$s: tag, %2$s: email
				esc_attr_x( 'Remove {{a tag:%1$s}} from {{a subscriber:%2$s}}', 'Drip', 'uncanny-automator' ),
	...
	protected function process_action( $user_id, $action_data, $recipe_id, $args, $parsed ) {
		...
		$this->api->remove_tag( $email, $tag, $action_data );
