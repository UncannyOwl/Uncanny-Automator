...
/**
 * Github Add Label to Issue or Pull Request Action.
 *
 ...
 */
class GITHUB_ADD_LABEL_TO_ISSUE_PR extends \Uncanny_Automator\Recipe\App_Action {
	...
	public function setup_action() {
		...
		$this->set_action_code( 'GITHUB_ADD_LABEL_TO_ISSUE_PR_CODE' );
		$this->set_action_meta( 'GITHUB_ADD_LABEL_TO_ISSUE_PR_META' );
		$this->set_requires_user( false );
		$this->set_is_pro( false );
		$this->set_support_link( Automator()->get_author_support_link( $this->get_action_code(), 'knowledge-base/github-integration/' ) );
		$this->set_sentence(
			sprintf(
				// translators: %1$s is the label name, %2$s is the issue or PR title.
				esc_attr_x( "Add {{a label:%1\$s}} to {{an issue or pull request:%2\$s}} in a repository", 'GitHub', 'uncanny-automator' ),
				$this->get_action_meta(),
				$this->issue_pr_meta_key . ':' . $this->get_action_meta(),
			)
		);
		$this->set_readable_sentence( esc_attr_x( "Add {{a label}} to {{an issue or pull request}} in a repository", 'GitHub', 'uncanny-automator' ) );
	...
	public function options() {
		return array(
			...
			$this->helpers->get_label_option_config( $this->get_action_meta(), $this->repo_meta_key, true ),
		);
	}
	...
	protected function process_action( $user_id, $action_data, $recipe_id, $args, $parsed ) {
		...
		$body = array(
			'action' => 'add_label_to_issue_or_pr',
		...
		$this->api->validate_action_response_status(
			$response,
			200,
			esc_html_x( 'Failed to add label to issue or pull request.', 'GitHub', 'uncanny-automator' )
		);
