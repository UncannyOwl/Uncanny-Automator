...
/**
 * Trigger: A pull request is opened in {a repository}
 *
 ...
 */
class PULL_REQUEST_OPENED_IN_REPO extends \Uncanny_Automator\Recipe\App_Trigger {
	...
	protected function setup_trigger() {
		...
		$this->set_trigger_code( 'PULL_REQUEST_OPENED_IN_REPO' );
		$this->set_trigger_meta( 'PULL_REQUEST_OPENED_IN_REPO_META' );
		$this->set_sentence(
			sprintf(
				// translators: %1$s: Repository name.
				esc_attr_x( 'A pull request is opened in {{a repository:%1$s}}', 'GitHub', 'uncanny-automator' ),
				$this->get_trigger_meta()
			)
		);
		$this->set_readable_sentence( esc_attr_x( 'A pull request is opened in {{a repository}}', 'GitHub', 'uncanny-automator' ) );
	...
	public function validate( $trigger, $hook_args ) {
		// Check if the trigger, hook args, and meta key are set, return false if not.
