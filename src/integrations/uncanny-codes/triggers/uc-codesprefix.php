 ...
 */
class UC_CODESPREFIX extends \Uncanny_Automator\Recipe\Trigger {
	...
	public static function definition() {
		return self::new_definition( 'UCPREFIX', 'UNCANNYCODE' )
	...
	protected function setup_trigger() {
		...
		// translators: %1$s is the prefix.
		$this->set_sentence( sprintf( esc_html_x( 'A user redeems a code with a {{specific:%1$s}} prefix', 'Uncanny Codes', 'uncanny-automator' ), $this->get_trigger_meta() ) );
		$this->set_readable_sentence( esc_html_x( 'A user redeems a code with a {{specific}} prefix', 'Uncanny Codes', 'uncanny-automator' ) );
	}
	...
	public function options() {
		return array(
			array(
				...
				'input_type'            => 'select',
				'required'              => true,
				'remote_data'           => $this->item_helpers->remote_data_load_config( 'prefixes' ),
				'options'               => array(),
			),
		);
	}
	...
	public function validate( $trigger, $hook_args ) {
		...
		$selected_prefix = $trigger['meta'][ $this->get_trigger_meta() . '_readable' ] ?? '';

		$prefix = $this->get_prefix_by_coupon( $coupon_id );

		if ( (string) $prefix !== (string) $selected_prefix ) {
			return false;
		}
	...
	/**
	 * Get prefix by coupon ID.
	 *
	 * @param int $coupon_id The coupon ID.
	 *
	 * @return string
	 */
	private function get_prefix_by_coupon( $coupon_id ) {

		global $wpdb;

		return (string) $wpdb->get_var(
			$wpdb->prepare(
				"SELECT g.prefix FROM `{$wpdb->prefix}uncanny_codes_groups` g LEFT JOIN `{$wpdb->prefix}uncanny_codes_codes` c ON g.ID = c.code_group WHERE c.ID = %d",
				$coupon_id
			)
		);
	}
