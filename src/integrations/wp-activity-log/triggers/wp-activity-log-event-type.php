...
/**
 * Class Wp_Activity_Log_Event_Type
 *
 * Fires when WP Activity Log records an event of the selected type (e.g. all
 * 'login' events, all 'deleted' events).
 *
 * @package Uncanny_Automator\Integrations\Wp_Activity_Log
 *
 * @property Wp_Activity_Log_Helpers $item_helpers
 */
class Wp_Activity_Log_Event_Type extends Trigger {
	...
	public static function definition() {
		return self::new_definition( 'WP_ACTIVITY_LOG_EVENT_TYPE', 'WP_ACTIVITY_LOG' )
	...
	protected function setup_trigger() {
		...
		$this->set_sentence(
			sprintf(
				// translators: %1$s: Event type.
				esc_html_x( 'An event of {{a type:%1$s}} is logged', 'WP Activity Log', 'uncanny-automator' ),
				$this->get_trigger_meta()
			)
		);
		$this->set_readable_sentence( esc_html_x( 'An event of {{a type}} is logged', 'WP Activity Log', 'uncanny-automator' ) );
	}
	...
	public function options() {
		return array(
			array(
				...
				'label'       => esc_html_x( 'Event type', 'WP Activity Log', 'uncanny-automator' ),
				'input_type'  => 'select',
				'required'    => true,
				'options'     => array(),
				'remote_data' => $this->item_helpers->remote_data_load_config( 'event_types' ),
			),
		);
	}
	...
	public function validate( $trigger, $hook_args ) {
		...
		if ( ! is_array( $data ) || ! isset( $data['EventType'] ) ) {
			return false;
		}

		$selected = $trigger['meta'][ $this->get_trigger_meta() ] ?? Wp_Activity_Log_Helpers::ANY;

		if ( Wp_Activity_Log_Helpers::ANY !== (string) $selected && (string) $data['EventType'] !== (string) $selected ) {
			return false;
		}
