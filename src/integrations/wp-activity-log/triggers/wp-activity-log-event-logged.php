...
/**
 * Class Wp_Activity_Log_Event_Logged
 *
 * Fires when WP Activity Log records an event. The selectable event-code
 * dropdown includes an "Any event" sentinel, so this one trigger covers both
 * "any event" and "a specific event" use cases.
 *
 * @package Uncanny_Automator\Integrations\Wp_Activity_Log
 *
 * @property Wp_Activity_Log_Helpers $item_helpers
 */
class Wp_Activity_Log_Event_Logged extends Trigger {
	...
	public static function definition() {
		return self::new_definition( 'WP_ACTIVITY_LOG_EVENT_LOGGED', 'WP_ACTIVITY_LOG' )
	...
	protected function setup_trigger() {
		...
		$this->set_sentence(
			sprintf(
				...
				esc_html_x( '{{An event:%1$s}} is logged', 'WP Activity Log', 'uncanny-automator' ),
				$this->get_trigger_meta()
			)
		);
		$this->set_readable_sentence( esc_html_x( '{{An event}} is logged', 'WP Activity Log', 'uncanny-automator' ) );
	}
	...
	public function options() {
		return array(
			array(
				...
				'label'       => esc_html_x( 'Event', 'WP Activity Log', 'uncanny-automator' ),
				'input_type'  => 'select',
				'required'    => true,
				'options'     => array(),
				'remote_data' => $this->item_helpers->remote_data_search_config( 'events' ),
			),
		);
	}
	...
	public function validate( $trigger, $hook_args ) {
		...
		$type     = (int) $hook_args[1];
		$selected = $trigger['meta'][ $this->get_trigger_meta() ] ?? Wp_Activity_Log_Helpers::ANY;

		if ( Wp_Activity_Log_Helpers::ANY !== (string) $selected && intval( $selected ) !== $type ) {
			return false;
		}
