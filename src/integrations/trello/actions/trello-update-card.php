 ...
 */
class TRELLO_UPDATE_CARD extends \Uncanny_Automator\Recipe\App_Action {
	 ...
	 * @var bool
	 */
	protected $is_update_action = true;
	...
	public function setup_action() {
		...
		$this->set_sentence(
			sprintf(
				...
				esc_attr_x( 'Update {{a card:%1$s}}', 'Trello', 'uncanny-automator' ),
				$this->get_action_meta()
			)
		);
		$this->set_readable_sentence( esc_attr_x( 'Update {{a card}}', 'Trello', 'uncanny-automator' ) );
	...
	protected function process_action( $user_id, $action_data, $recipe_id, $args, $parsed ) {
		...
		$response = $this->api->api_request(
			...
			$action_data,
			array( 'error_message' => esc_html_x( 'Unable to update card.', 'Trello', 'uncanny-automator' ) )
		);
