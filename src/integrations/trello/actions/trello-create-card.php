...
class TRELLO_CREATE_CARD {
	...
	public function setup_action() {
		...
		$this->set_sentence( sprintf( esc_attr__( 'Create {{a card:%1$s}}', 'uncanny-automator' ), $this->get_action_meta() ) );
		$this->set_readable_sentence( esc_attr__( 'Create {{a card}}', 'uncanny-automator' ) );
	...
	protected function process_action( $user_id, $action_data, $recipe_id, $args, $parsed ) {
		...
		try {
			...
			$response = $this->functions->api->create_card( $card );
