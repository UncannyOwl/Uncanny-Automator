...
class UOA_RECIPESUCCESSCOMPLETEDTIMES {
	...
	public function __construct() {
		$this->trigger_code = 'UOARECIPESUCCESSRUNS';
	...
	public function define_trigger() {
		...
		$trigger = array(
			...
			'sentence'            => sprintf( esc_attr__( '{{A recipe:%1$s}} completes {{a number of:%2$s}} time(s)', 'uncanny-automator' ), $this->trigger_meta, $this->num_times ),
			/* translators: Logged-in trigger - Uncanny Automator */
			'select_option_name'  => esc_attr__( '{{A recipe}} completes {{a number of}} time(s)', 'uncanny-automator' ),
	...
	public function on_completion( $recipe_id, $user_id, $recipe_log_id, $args ) {
		...
		$completed_count = $wpdb->get_var( $wpdb->prepare( "SELECT count(ID) FROM $table_name WHERE automator_recipe_id = %d AND completed = %d", $recipe_id, 1 ) ); //phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
