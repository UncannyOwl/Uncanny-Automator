 ...
 */
class AUDIENCE_ADDUSERTAG extends \Uncanny_Automator\Recipe\App_Action {
	...
	protected function setup_action() {
		...
		$this->set_action_code( 'MCHIMPAUDIENCEADDUSERTAG' );
		$this->set_action_meta( 'AUDIENCEADDUSERTAG' );
		$this->set_is_pro( false );
		$this->set_requires_user( true );
		$this->set_background_processing( true );
		$this->set_support_link( Automator()->get_author_support_link( $this->get_action_code(), 'knowledge-base/mailchimp/' ) );
		$this->set_readable_sentence( esc_html_x( 'Add {{a tag}} to the user', 'Mailchimp', 'uncanny-automator' ) );
		$this->set_sentence(
			sprintf(
				// translators: %1$s is the tag
				esc_html_x( 'Add {{a tag:%1$s}} to the user', 'Mailchimp', 'uncanny-automator' ),
	...
	protected function process_action( $user_id, $action_data, $recipe_id, $args, $parsed ) {
		...
		$this->api->add_tag_to_contact( $list_id, $user_email, array( $tag ) );
