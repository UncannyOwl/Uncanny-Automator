 ...
 */
class CONVERTKIT_FORM_SUBSCRIBER_ADD extends \Uncanny_Automator\Recipe\App_Action {
	...
	protected function setup_action() {
		...
		$this->set_readable_sentence( esc_attr_x( 'Add {{a subscriber}} to {{a form}}', 'ConvertKit', 'uncanny-automator' ) );
		$this->set_sentence(
			sprintf(
				// translators: %1$s is the email address, %2$s is the form name
				esc_attr_x( 'Add {{a subscriber:%1$s}} to {{a form:%2$s}}', 'ConvertKit', 'uncanny-automator' ),
	...
	public function options() {
		return array(
			array(
				...
				'label'       => esc_attr_x( 'Form', 'ConvertKit', 'uncanny-automator' ),
				'input_type'  => 'select',
				'options'     => array(),
				'required'    => true,
				'ajax'        => array(
	...
	protected function process_action( $user_id, $action_data, $recipe_id, $args, $parsed ) {
		...
		$form_id    = absint( $parsed[ $this->get_action_meta() ] ?? 0 );
		$email      = $this->helpers->require_valid_email( $parsed['EMAIL'] ?? '' );
		$first_name = sanitize_text_field( $parsed['FIRST_NAME'] ?? '' );

		if ( empty( $form_id ) ) {
			throw new \Exception(
				esc_html_x( 'Please provide a valid form.', 'ConvertKit', 'uncanny-automator' )
			);
		}
