...
/**
 * EDD User State Token
 *
 * @package Uncanny_Automator\Integrations\Easy_Digital_Downloads
 */
class EDD_User_State_Token extends Universal_Token {
	 ...
	 * @var string
	 */
	const EDD_USER_STATE = 'EDD_USER_STATE';
	...
	public function setup() {
		...
		$this->id            = self::EDD_USER_STATE;
		$this->name          = esc_attr_x( 'User state', 'EDD', 'uncanny-automator' );
	...
	public function parse_integration_token( $default_return, $pieces, $recipe_id, $trigger_data, $user_id, $replace_args ) {
		...
		// Handle EDD User State token
		if ( self::EDD_USER_STATE === $token_id ) {
			$address_data = $this->edd_helpers->get_user_address_data( $user_id );

			return isset( $address_data['state'] ) ? $address_data['state'] : '';
		}
