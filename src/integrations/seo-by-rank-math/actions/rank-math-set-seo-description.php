...
/**
 * Class Rank_Math_Set_Seo_Description
 *
 * @package Uncanny_Automator
 * @method \Uncanny_Automator\Integrations\Seo_By_Rank_Math\Seo_By_Rank_Math_Helpers get_item_helpers()
 */
class Rank_Math_Set_Seo_Description extends \Uncanny_Automator\Recipe\Action {
	...
	protected function setup_action() {
		...
		$this->set_action_code( 'RANK_MATH_SET_SEO_DESCRIPTION' );
		$this->set_action_meta( 'RANK_MATH_POST' );
		$this->set_is_pro( false );
		$this->set_requires_user( false );
		$this->set_background_processing( true );
		// translators: %1$s is the post, %2$s is the SEO description value.
		$this->set_sentence( sprintf( esc_html_x( "Set {{a post's:%1\$s}} SEO description to {{a specific value:%2\$s}}", 'Rank Math SEO', 'uncanny-automator' ), $this->get_action_meta(), 'SEO_DESCRIPTION_VALUE:' . $this->get_action_meta() ) );
		$this->set_readable_sentence( esc_html_x( "Set {{a post's}} SEO description to {{a specific value}}", 'Rank Math SEO', 'uncanny-automator' ) );
		$this->set_action_tokens(
			array(
				...
				'SEO_DESCRIPTION' => array(
					'name' => esc_html_x( 'SEO description', 'Rank Math SEO', 'uncanny-automator' ),
					'type' => 'text',
				),
			),
	...
	public function options() {
		...
		$options[] = array(
			...
			'label'       => esc_html_x( 'SEO description', 'Rank Math SEO', 'uncanny-automator' ),
			'input_type'  => 'textarea',
			'required'    => true,
		);
