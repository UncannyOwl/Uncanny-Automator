...
/**
 * Class Rank_Math_Delete_Focus_Keyword
 *
 * @package Uncanny_Automator
 * @method \Uncanny_Automator\Integrations\Seo_By_Rank_Math\Seo_By_Rank_Math_Helpers get_item_helpers()
 */
class Rank_Math_Delete_Focus_Keyword extends \Uncanny_Automator\Recipe\Action {
	...
	protected function setup_action() {
		...
		$this->set_action_code( 'RANK_MATH_DELETE_FOCUS_KEYWORD' );
		$this->set_action_meta( 'RANK_MATH_POST' );
		$this->set_is_pro( false );
		$this->set_requires_user( false );
		$this->set_background_processing( true );
		// translators: %1$s is the post.
		$this->set_sentence( sprintf( esc_html_x( "Delete {{a post's:%1\$s}} focus keyword", 'Rank Math SEO', 'uncanny-automator' ), $this->get_action_meta() ) );
		$this->set_readable_sentence( esc_html_x( "Delete {{a post's}} focus keyword", 'Rank Math SEO', 'uncanny-automator' ) );
