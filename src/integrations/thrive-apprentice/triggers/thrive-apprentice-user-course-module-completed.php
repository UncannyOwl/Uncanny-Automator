...
class THRIVE_APPRENTICE_USER_COURSE_MODULE_COMPLETED {
	...
	public function setup_trigger() {
		...
		$this->set_sentence(
			sprintf(
				...
				esc_html__( 'A user completes {{a module:%1$s}} in {{a course:%2$s}}', 'uncanny-automator' ),
		...
		$this->set_readable_sentence(
			/* Translators: Trigger sentence */
			esc_html__( 'A user completes {{a module}} in {{a course}}', 'uncanny-automator' )
		);
	...
	public function validate_trigger( ...$args ) {
		...
		return ! empty( $module ) && ! empty( $user );
	...
	public function validate_conditions( ...$args ) {
		...
		$matching_recipes_triggers = $this->find_all( $this->trigger_recipes() )
			...
			->match( array( absint( $lesson['module_id'] ), absint( $lesson['course_id'] ) ) )
