...
/**
 * Class AFFWP_REFERRALCOUNTREACHES
 *
...
class AFFWP_REFERRALCOUNTREACHES {
	...
	public function __construct() {
		$this->trigger_code = 'AFFWPREFERRALCOUNTREACHES';
		$this->trigger_meta = 'AFFWPUNPAIDREFERRALCOUNT';
		$this->define_trigger();
	}
	...
	public function define_trigger() {
		...
		$trigger = array(
			...
			'sentence'            => sprintf( esc_html_x( "An affiliate's unpaid referral count is {{greater than, less than, or equal to:%1\$s}} {{a specific number:%2\$s}}", 'Affiliate Wp', 'uncanny-automator' ), 'NUMBERCOND', $this->trigger_meta ),
			/* translators: Logged-in trigger - Affiliate WP */
			'select_option_name'  => esc_html_x( "An affiliate's unpaid referral count is {{greater than, less than, or equal to}} {{a specific number}}", 'Affiliate Wp', 'uncanny-automator' ),
			'action'              => 'affwp_set_referral_status',
			'priority'            => 99,
			'accepted_args'       => 3,
			'validation_function' => array( $this, 'affwp_referral_count_reaches' ),
			...
			array(
				'options' => array(
					...
					array(
						...
						'label'       => esc_html_x( 'Number of unpaid referrals', 'Affiliate Wp', 'uncanny-automator' ),
						'input_type'  => 'int',
						'required'    => true,
					),
				),
	...
	/**
	 * Fires when a referral status changes.
	 *
	 * @param int    $referral_id The referral ID.
	 * @param string $new_status  The new status.
	 * @param string $old_status  The old status.
	 */
	public function affwp_referral_count_reaches( $referral_id, $new_status, $old_status ) {

		if ( 'unpaid' !== (string) $new_status || (string) $new_status === (string) $old_status ) {
			return;
		}
		...
		$unpaid_count = absint( affwp_count_referrals( $affiliate_id, 'unpaid' ) );
		...
		foreach ( $recipes as $recipe_id => $recipe ) {
			foreach ( $recipe['triggers'] as $trigger ) {
				...
				if ( Automator()->utilities->match_condition_vs_number( $required_condition[ $recipe_id ][ $trigger_id ], $required_count[ $recipe_id ][ $trigger_id ], $unpaid_count ) ) {
					$matched_recipe_ids[] = array(
						'recipe_id'  => $recipe_id,
			...
			if ( $args ) {
				foreach ( $args as $result ) {
					if ( true === $result['result'] ) {
						...
						$trigger_meta['meta_value'] = maybe_serialize( $unpaid_count );
