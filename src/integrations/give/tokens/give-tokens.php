...
class Give_Tokens {
						...
						foreach ( $trigger_data as $trigger ) {
							...
							if ( ! empty( $field_key ) && ! in_array( $field_key, $billing_fields, true ) ) {
								...
								if ( ! empty( $meta_value ) ) {
									...
									$form_fields = $uncanny_automator->helpers->recipe->give->get_form_fields_and_ffm( $meta_value['give_form_id'] );
									$form_field  = isset( $form_fields[ $field_key ] ) ? $form_fields[ $field_key ] : [];
									if ( ! empty( $form_field ) ) {
										if ( ! empty( $meta_value ) ) {
	 ...
	 * @return array
	 */
	public function givewp_possible_tokens( $tokens = [], $args = [] ) {
		...
		$form_fields = $uncanny_automator->helpers->recipe->give->get_form_fields_and_ffm( $form_id );
