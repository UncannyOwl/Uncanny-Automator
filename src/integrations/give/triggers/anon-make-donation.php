...
class ANON_MAKE_DONATION {
	...
	public function define_trigger() {
		...
		$trigger = array(
			...
			'sentence'            => sprintf( esc_html_x( 'A donation is made via {{a form:%1$s}}', 'Give', 'uncanny-automator' ), $this->trigger_meta ),
			/* translators: Logged-in trigger - GiveWP */
			'select_option_name'  => esc_html_x( 'A donation is made via {{a form}}', 'Give', 'uncanny-automator' ),
			...
			array(
				'options' => array(
					Automator()->helpers->recipe->give->options->list_all_give_forms(
						esc_html_x( 'Form', 'Give', 'uncanny-automator' ),
						$this->trigger_meta,
					),
				),
			)
	...
	public function givewp_make_donation( $payment_id, $status, $old_status ) {
		...
		$payment_data['give_comment']    = esc_html_x( '-', 'Give', 'uncanny-automator' );
