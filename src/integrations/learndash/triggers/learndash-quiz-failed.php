 ...
 */
class LD_FAILQUIZ extends \Uncanny_Automator\Recipe\Trigger {
	...
	public static function definition() {
		return self::new_definition( 'LD_FAILQUIZ', 'LD' )
			->trigger_meta( 'LDQUIZ' )
	...
	protected function setup_trigger() {
		...
		$this->set_readable_sentence( esc_html_x( 'A user fails {{a quiz}}', 'LearnDash', 'uncanny-automator' ) );
	}
	 ...
	 *
	 * Opposite of LD_PASSQUIZ — fires when the user FAILS the quiz.
	 *
	...
	public function validate( $trigger, $hook_args ) {
		...
		if ( did_action( 'learndash_quiz_submitted' ) ) {
			...
			// For fail: return false if passed is WP_Error OR if quiz was passed (non-empty).
