...
class Fluent_Boards_Task_Assignee_Added extends Trigger {
	...
	public static function definition() {
		return self::new_definition( 'FLUENT_BOARDS_TASK_ASSIGNEE_ADDED', 'FLUENT_BOARDS' )
			->trigger_meta( Fluent_Boards_Helpers::BOARD )
			->trigger_type( 'user' )
			->hook( 'fluent_boards/task_assignee_added', 10, 2 );
	}
	...
	protected function setup_trigger() {
		...
		$this->set_sentence(
			sprintf(
				...
				esc_html_x( 'A user is assigned to a task on {{a board:%1$s}}', 'Fluent Boards', 'uncanny-automator' ),
				$this->get_trigger_meta()
			)
		);
		$this->set_readable_sentence( esc_html_x( 'A user is assigned to a task on {{a board}}', 'Fluent Boards', 'uncanny-automator' ) );
	}
