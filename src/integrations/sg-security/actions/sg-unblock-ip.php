...
class Sg_Unblock_Ip extends \Uncanny_Automator\Recipe\Action {
	...
	protected function setup_action() {
		...
		$this->set_sentence( sprintf( esc_html_x( 'Unblock {{an IP address:%1$s}}', 'Security Optimizer', 'uncanny-automator' ), $this->get_action_meta() ) );
		$this->set_readable_sentence( esc_html_x( 'Unblock {{an IP address}}', 'Security Optimizer', 'uncanny-automator' ) );
	...
	protected function process_action( $user_id, $action_data, $recipe_id, $args, $parsed ) {
		...
		if ( null === $visitor ) {
			$this->add_log_error( sprintf( esc_html_x( 'No visitor record found for IP address: %s', 'Security Optimizer', 'uncanny-automator' ), $ip ) );
			return false;
		}
