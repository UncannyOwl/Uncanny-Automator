...
/**
 * Class DISCORD_ASSIGN_ROLE_TO_MEMBER
 *
 * @package Uncanny_Automator
 */
class DISCORD_ASSIGN_ROLE_TO_MEMBER extends \Uncanny_Automator\Recipe\Action {
	 ...
	 * @var string
	 */
	public $prefix = 'DISCORD_ASSIGN_ROLE_TO_MEMBER';
	...
	public function setup_action() {
		...
		$this->set_sentence(
			sprintf(
				...
				esc_attr_x( 'Assign {{a role:%1$s}} to {{a member:%2$s}}', 'Discord', 'uncanny-automator' ),
				$this->get_action_meta(),
				'MEMBER:' . $this->get_action_meta()
			)
		);
		$this->set_readable_sentence( esc_attr_x( 'Assign {{a role}} to {{a member}}', 'Discord', 'uncanny-automator' ) );
	...
	protected function process_action( $user_id, $action_data, $recipe_id, $args, $parsed ) {
		...
		if ( 204 !== $status_code ) {
			throw new Exception( _x( 'Error adding role to user.', 'Discord', 'uncanny-automator' ) );
		}
