 ...
 * Studiocart Trigger
 */
class STUDIOCART_USERORDERREFUNDED {
	...
	public function __construct() {
		$this->trigger_code = 'USERORDERREFUNDED';
	...
	public function define_trigger() {
		$trigger = array(
			...
			'sentence'            => sprintf( esc_attr__( "A user's order for {{a product:%1\$s}} is refunded", 'uncanny-automator' ), $this->trigger_meta ),
			/* translators: Logged-in trigger - Studiocart */
			'select_option_name'  => esc_attr__( "A user's order for {{a product}} is refunded", 'uncanny-automator' ),
	 ...
	 */
	public function order_refunded( $status, $order_data, $order_type ) {
		...
		$payment_status = get_post_meta( $order_data['ID'], '_sc_status', true );
		if ( 'refunded' !== (string) $payment_status ) {
			return;
		}
