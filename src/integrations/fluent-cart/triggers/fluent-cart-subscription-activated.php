...
/**
 * Class FLUENT_CART_SUBSCRIPTION_ACTIVATED
 *
 * Fires on `fluent_cart/subscription_activated`.
 *
 * @package Uncanny_Automator\Integrations\Fluent_Cart
 *
 * @property Fluent_Cart_Helpers $item_helpers
 */
class Fluent_Cart_Subscription_Activated extends Trigger {

	/**
	 * @return \Uncanny_Automator\Recipe\Trigger_Definition
	 */
	public static function definition() {
		return self::new_definition( 'FLUENT_CART_SUBSCRIPTION_ACTIVATED', 'FLUENT_CART' )
			->trigger_type( 'user' )
			->trigger_meta( 'FLUENT_CART_PRODUCT' )
			->hook( 'fluent_cart/subscription_activated', 10, 1 );
	}
	...
	protected function setup_trigger() {
		...
		$this->set_sentence(
			sprintf(
				...
				esc_html_x( "A user's subscription to {{a product:%1\$s}} is activated", 'FluentCart', 'uncanny-automator' ),
				$this->get_trigger_meta()
			)
		);
		$this->set_readable_sentence( esc_html_x( "A user's subscription to {{a product}} is activated", 'FluentCart', 'uncanny-automator' ) );
	}
	...
	public function define_tokens( $trigger, $tokens ) {
		return array_merge( $tokens, $this->item_helpers->subscription_tokens(), $this->item_helpers->customer_tokens() );
	}
