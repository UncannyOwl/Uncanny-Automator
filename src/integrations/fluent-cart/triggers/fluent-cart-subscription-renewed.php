...
/**
 * Class FLUENT_CART_SUBSCRIPTION_RENEWED
 *
 * Fires on `fluent_cart/subscription_renewed`.
 *
 * @package Uncanny_Automator\Integrations\Fluent_Cart
 *
 * @property Fluent_Cart_Helpers $item_helpers
 */
class Fluent_Cart_Subscription_Renewed extends Trigger {

	/**
	 * @return \Uncanny_Automator\Recipe\Trigger_Definition
	 */
	public static function definition() {
		return self::new_definition( 'FLUENT_CART_SUBSCRIPTION_RENEWED', 'FLUENT_CART' )
			->trigger_type( 'user' )
			->trigger_meta( 'FLUENT_CART_PRODUCT' )
			->hook( 'fluent_cart/subscription_renewed', 10, 1 );
	}
	...
	protected function setup_trigger() {
		...
		$this->set_sentence(
			sprintf(
				...
				esc_html_x( "A user's subscription to {{a product:%1\$s}} is renewed", 'FluentCart', 'uncanny-automator' ),
				$this->get_trigger_meta()
			)
		);
		$this->set_readable_sentence( esc_html_x( "A user's subscription to {{a product}} is renewed", 'FluentCart', 'uncanny-automator' ) );
	}
	...
	public function define_tokens( $trigger, $tokens ) {
		return array_merge(
			...
			array(
				array(
					'tokenId'   => 'FLUENT_CART_RENEWAL_ORDER_ID',
					'tokenName' => esc_html_x( 'Renewal order ID', 'FluentCart', 'uncanny-automator' ),
					'tokenType' => 'int',
				),
			)
		);
	...
	public function hydrate_tokens( $trigger, $hook_args ) {
		...
		return array_merge(
			...
			array(
				'FLUENT_CART_RENEWAL_ORDER_ID' => (string) $this->item_helpers->prop( $order, 'id', '' ),
			)
		);
	}
}
