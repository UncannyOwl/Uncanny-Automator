...
/**
 * Class FLUENTCOMMUNITY_USER_COURSE_COMPLETED
 *
 * @package Uncanny_Automator
 */
class FLUENTCOMMUNITY_USER_COURSE_COMPLETED extends Trigger {

	protected $prefix = 'FLUENTCOMMUNITY_USER_COURSE_COMPLETED';
	...
	protected function setup_trigger() {
		...
		$this->add_action( 'fluent_community/course/completed' );
		$this->set_action_args_count( 2 );

		$this->set_sentence(
			sprintf(
				/* translators: %1$s - Course */
				esc_html_x( 'A user completes {{a course:%1$s}}', 'FluentCommunity', 'uncanny-automator' ),
				$this->get_trigger_meta()
			)
		);

		$this->set_readable_sentence(
			esc_html_x( 'A user completes {{a course}}', 'FluentCommunity', 'uncanny-automator' )
		);
	}
	/**
	 * Options.
