...
class ANON_FORM_SUBMITTED extends \Uncanny_Automator\Recipe\Trigger {
	...
	protected function setup_trigger() {
		...
		$this->set_trigger_code( 'SURE_ANON_SUBMITS_FORM' );
		$this->set_trigger_meta( 'SURE_FORMS' );
		$this->set_trigger_type( 'anonymous' );
		// translators: 1: Form name
		$this->set_sentence( sprintf( esc_attr_x( '{{A form:%1$s}} is submitted', 'Sure Forms', 'uncanny-automator' ), $this->get_trigger_meta() ) );
		$this->set_readable_sentence( esc_attr_x( '{{A form}} is submitted', 'Sure Forms', 'uncanny-automator' ) );
