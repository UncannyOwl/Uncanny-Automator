 ...
 */
class SENDY_UNSUBSCRIBE_LIST_CONTACT extends \Uncanny_Automator\Recipe\App_Action {
	...
	public function setup_action() {
		...
		$this->set_sentence(
			...
			// translators: %1$s: Contact Email,%2$s: List Name
				esc_attr_x( 'Unsubscribe {{a contact:%1$s}} from {{a list:%2$s}}', 'Sendy', 'uncanny-automator' ),
	...
	protected function process_action( $user_id, $action_data, $recipe_id, $args, $parsed ) {
		...
		try {
			$this->api->unsubscribe_contact_from_list( $email, $list, $action_data );
		} catch ( \Exception $e ) {
			// A contact that isn't on the list is already unsubscribed — complete
