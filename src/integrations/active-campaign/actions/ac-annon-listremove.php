 ...
 */
class AC_ANNON_LISTREMOVE extends \Uncanny_Automator\Recipe\App_Action {
	 ...
	 * @var string
	 */
	protected $prefix = 'AC_ANNON_LISTREMOVE';
	...
	protected function setup_action() {
		...
		$this->set_sentence(
			sprintf(
				...
				esc_attr_x( 'Remove {{a contact:%1$s}} from {{a list:%2$s}}', 'ActiveCampaign', 'uncanny-automator' ),
				$this->prefix . '_CONTACT_ID:' . $this->get_action_meta(),
				$this->get_action_meta()
			)
		);
		$this->set_readable_sentence( esc_attr_x( 'Remove {{a contact}} from {{a list}}', 'ActiveCampaign', 'uncanny-automator' ) );
	...
	protected function process_action( $user_id, $action_data, $recipe_id, $args, $parsed ) {
		...
		$this->api->remove_contact_from_list( $email, $list_id, $action_data );
