...
final class MediaController
{
    ...
    public function upload(\WP_REST_Request $request): \WP_REST_Response|\WP_Error
    {
        ...
        try {
            ...
            if (is_wp_error($insertedAttachment) || (int) $insertedAttachment <= 0) {
                $this->cleanupFailedUpload(0, $upload['file']);
            ...
            if (is_wp_error($metadata) || !is_array($metadata)) {
                $this->cleanupFailedUpload($attachmentId, $upload['file']);
     ...
     */
    private function cleanupFailedUpload(int $attachmentId, string $file): void
    {
        if ($attachmentId > 0 && function_exists('wp_delete_attachment')) {
