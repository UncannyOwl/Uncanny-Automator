<?php
namespace Uncanny_Automator\Deprecated;

/**
 * Class Legacy_Token_Parser
 *
...
class Legacy_Token_Parser {
	...
	public function parse_vars( $args, $trigger_args = array() ) {
		...
		$field_text = $this->parse_inner_tokens( $field_text, $args, $trigger_args );
	...
	public function parse_inner_tokens( $field_text, $args, $trigger_args ) {
		...
		$replacements_count = preg_match_all( '/{{((?>[^{}]+|(?R))*)}}/', $field_text, $arr );
		...
		foreach ( $matches as $match ) {
			...
			$parsed_match = $this->parse_inner_tokens( $match, $args, $trigger_args );

			if ( false !== strpos( $parsed_match, ':' ) ) {

				// This section is for user meta tokens.
				if ( preg_match( '/(USERMETA)/', $parsed_match ) ) {
		...
		$output = preg_replace( '/({{(.+?)}})/', '$2', $field_text );
